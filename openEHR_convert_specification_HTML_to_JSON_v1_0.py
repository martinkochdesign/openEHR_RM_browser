...
def download_RM(outputdir):
	...
	release_versions = ['1.1.0','1.0.4','1.0.3','1.0.2'] 

	for v in release_versions:
		RM['RM'][v]={}

		URL_list = [
		'https://specifications.openehr.org/releases/BASE/Release-'+v+'/base_types.html',
		'https://specifications.openehr.org/releases/RM/Release-'+v+'/common.html',
		'https://specifications.openehr.org/releases/RM/Release-'+v+'/data_structures.html',
		'https://specifications.openehr.org/releases/RM/Release-'+v+'/data_types.html',
		'https://specifications.openehr.org/releases/RM/Release-'+v+'/demographic.html',
		'https://specifications.openehr.org/releases/RM/Release-'+v+'/ehr_extract.html',
		'https://specifications.openehr.org/releases/RM/Release-'+v+'/ehr.html',
		'https://specifications.openehr.org/releases/RM/Release-'+v+'/integration.html',
		'https://specifications.openehr.org/releases/RM/Release-'+v+'/support.html',
		'https://specifications.openehr.org/releases/BASE/Release-'+v+'/foundation_types.html',
		'https://specifications.openehr.org/releases/BASE/Release-'+v+'/resource.html'
		]
			...
			for table in results:
				...
				for i in range(0,len(table_rows)):
					...
					if len(header_cells) == 1 and header_cells[0].text == "Inherit":
						RM['RM'][v][RM_class]['inherit']=table_cells[0].text.strip()
					if len(header_cells) == 1 and header_cells[0].text == "Description":
						RM['RM'][v][RM_class]['description']=table_cells[0].text.strip()
					if len(header_cells) == 2 and (header_cells[0].text == "Class" or header_cells[0].text == "Interface" or header_cells[0].text == "Enumeration"):
						...
						RM['RM'][v][RM_class]={}
						RM['RM'][v][RM_class]['type'] = header_cells[0].text
						if raw_title.find('(abstract)')>-1:
							RM['RM'][v][RM_class]['abstract']='true'
					if len(header_cells) == 3 and header_cells[0].text == "Attributes":
						RM['RM'][v][RM_class]['attributes']={}
								...
								if len(header_cells)!=0:
									...
									if 	header_cells[0].text.find('1..1')>-1 or header_cells[0].text.find('0..1')>-1:
										...
										if header_cells[0].text.find('1..1')>-1:
											...
											existence = '0..1'
										RM['RM'][v][RM_class]['attributes'][name]={}
										RM['RM'][v][RM_class]['attributes'][name]['existence']= existence
										RM['RM'][v][RM_class]['attributes'][name]['type'] = types

										RM['RM'][v][RM_class]['attributes'][name]['description'] = table_cells[1].text.strip()
									else:
										break
							else:
								break
						...
						for j in range(i+1,len(table_rows)):
							...
							if len(header_cells)==1:
								...
								if 	header_cells[0].text.find('1..1')>-1 or header_cells[0].text.find('0..1')>-1:
									...
									if name!='':
										RM['RM'][v][RM_class]['functions'][name]={}
										RM['RM'][v][RM_class]['functions'][name]['existence']= existence
										RM['RM'][v][RM_class]['functions'][name]['return_type'] = types
										RM['RM'][v][RM_class]['functions'][name]['description'] = table_cells[1].text.strip()
										RM['RM'][v][RM_class]['functions'][name]['prerequisite'] = prerequisite
								else:
									break
							else:
								break
		...
		classList = list(RM['RM'][v].keys())


		for i in range(len(classList)):
			RM_class = classList[i]
			if 'abstract' in RM['RM'][v][RM_class].keys():
				...
				for j in range(0,len(classList)):
					...
					if 'inherit' in RM['RM'][v][next_RM_class].keys():
						inherit_list = RM['RM'][v][next_RM_class]['inherit'].split(', ')
						if RM_class in inherit_list:
							print('Specialized class:', next_RM_class)
							if not 'specialization' in  RM['RM'][v][RM_class].keys():
								RM['RM'][v][RM_class]['specialization']=[]
							RM['RM'][v][RM_class]['specialization'].append(next_RM_class)
							if RM_class == 'DATA_VALUE' and 'DV_CODED_TEXT' not in RM['RM'][v][RM_class]['specialization']:
								RM['RM'][v][RM_class]['specialization'].append('DV_CODED_TEXT')
								RM['RM'][v][RM_class]['specialization'].append('DV_EHR_URI')
		...
		#for every class key...
		listOfClasses = list(RM['RM'][v].keys())
		for c in listOfClasses:
			#search in the dataframe
			df_search = df_out[df_out['class'] == c]
			if len(df_search)>0:
				#add url to dictionary
				RM['RM'][v][c]['url']=list(df_search['URL'])[0]
		#print(RM)
